        ...
        {loading ? <div className="spinner" /> : (
          <>
            {tab === 'classes' && (
              <div className="card" style={{ overflowX: 'auto' }}>
                <table className="admin-table">
                  <thead><tr><th>Class</th><th>Instructor</th><th>Location</th><th>Date</th><th>Spots</th><th>Status</th></tr></thead>
                  <tbody>
                    {classes.map(c => (
                      <tr key={c.id}>
            ...
            {tab === 'bookings' && (
              <div className="card" style={{ overflowX: 'auto' }}>
                <table className="admin-table">
                  <thead><tr><th>User</th><th>Class</th><th>Date</th><th>Status</th><th>Payment</th><th>Actions</th></tr></thead>
                  <tbody>
                    {bookings.map(b => (
                      <tr key={b.id}>
                        <td>{b.user_email}</td>
                        ...
                        <td>
                          <select value={b.payment_status} onChange={e => updatePayment(b.booking_id, e.target.value)}
                            style={{ padding: '4px 8px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)', fontSize: 13 }}>
                            <option value="pending">Pending</option><option value="paid">Paid</option><option value="comped">Comped</option>
                          </select>
                        </td>
                      </tr>
                    ))}
