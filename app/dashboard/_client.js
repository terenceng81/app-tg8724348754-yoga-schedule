                  ...
                  <div key={b.id} className="booking-row">
                    <div className="booking-info">
                      ...
                      <div className="booking-detail">
                        {format(parseISO(b.start_time), 'EEE, MMM d · h:mm a')} · {b.duration}min · {b.instructor_name} · {b.location_name}
                      </div>
                      <div style={{ marginTop: 4 }}>
                        <span className={`chip ${b.payment_status === 'paid' ? 'chip-success' : b.payment_status === 'comped' ? 'chip-accent' : 'chip-warning'}`}>
                          {b.payment_status}
                        </span>
                      </div>
                    </div>
                ...
                waitlist.map(w => (
                  <div key={w.id} className="booking-row">
                    <div className="booking-info">
                      ...
                      <div className="booking-detail">
                        {format(parseISO(w.start_time), 'EEE, MMM d · h:mm a')} · {w.instructor_name}
                      </div>
                    </div>
                ...
                history.map(b => (
                  <div key={b.id} className="booking-row">
                    <div className="booking-info">
                      <div className="booking-title">{b.class_type}</div>
                      <div className="booking-detail">
                        {format(parseISO(b.start_time), 'MMM d, yyyy · h:mm a')} · {b.instructor_name}
                      </div>
                    </div>
                    <span className="chip chip-success">{b.attended ? 'Attended' : 'Booked'}</span>
                  </div>
                ))
              )}
            </div>
