        ...
        {loading ? <div className="spinner" /> : (
          <>
            ...
            <div className="week-grid">
              {days.map(day => {
                const dayClasses = filtered.filter(c => isSameDay(parseISO(c.start_time), day))
                    ...
                    <div className="day-body">
                      ...
                      {dayClasses.map(cls => {
                        ...
                        return (
                          <div
                            key={cls.id}
                            className={`class-card ${full ? 'full' : ''}`}
                            onClick={() => setSelected(cls)}
                          >
                            <div className="class-card-name">{cls.class_type}</div>
                            <div className="class-card-time">
                              {format(parseISO(cls.start_time), 'h:mm a')} · {cls.duration}min
                            </div>
          ...
          <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setSelected(null)}>
            <div className="modal">
              ...
              <div className="detail-meta">
                <span className="chip chip-primary">{selected.difficulty}</span>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div className="detail-row">🕐 {format(parseISO(selected.start_time), 'EEEE, MMM d · h:mm a')}</div>
              </div>
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ height: 6, background: 'var(--bg)', borderRadius: 3, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${Math.min(100, (selected.confirmed_count / selected.capacity) * 100)}%`, background: selected.confirmed_count >= selected.capacity ? 'var(--error)' : 'var(--primary)', borderRadius: 3, transition: 'width 0.3s' }} />
                </div>
              </div>
              <div className="modal-actions">
                <button className="btn btn-secondary" onClick={() => setSelected(null)}>Close</button>
                <button
                  className={`btn ${selected.confirmed_count >= selected.capacity ? 'btn-accent' : 'btn-primary'} btn-lg`}
...
function AppShell({ children, ss }) {
  const { data: session } = ss ? { data: ss } : authClient.useSession()
